...
const MAX_POLL_DURATION_MS = 20 * 60 * 1000; // 最长等待 20 分钟
...
async function main() {
  ...
  while (Date.now() - startTime < MAX_POLL_DURATION_MS) {
    ...
    switch (status) {
      ...
      case "canceled":
        console.log("");
        console.log("SLIDE_FAILED");
        console.log("ERROR:generation_canceled - 幻灯片生成已取消");
        process.exit(1);
        break;
      case "not_found":
        console.log("");
        console.log("SLIDE_FAILED");
        console.log(
          "ERROR:session_not_found - session_id 不正确或已过期"
        );
        process.exit(1);
        break;
      case "in_progress":
  ...
  console.log("SLIDE_FAILED");
  console.log(
    `ERROR:timeout - 幻灯片生成超时（已等待 ${MAX_POLL_DURATION_MS / 60000} 分钟）`
  );
  console.log(`SESSION_ID:${sessionId}`);
  process.exit(1);
}

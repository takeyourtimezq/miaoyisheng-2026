...
// ── Slide 2: 目录 ──────────────────────────────────────────────
{
  ...
  const modules = [
    { num: "01", title: "行业现状", sub: "买方市场vs卖方市场·10年变迁·引流产品" },
    { num: "02", title: "销售心理", sub: "美业案例+世界品牌引流怎么做" },
    { num: "03", title: "话术体系", sub: "邀约·到店·转化·跟进全链路" },
    { num: "04", title: "团单详解", sub: "新团单产品+佣金+利弊分析+问题预判" },
  ...
  modules.forEach((m, i) => {
    ...
    s.addShape("rect", { x, y: y + 1.45, w: 2.9 * ((i + 1) / 5), h: 0.25, fill: { color: C.secondary }, line: { width: 0 } });
  });
  footer(s, 2);
}
...
// ── Slide 22: 引流团单利弊分析（新增）───────────────────────────
{
  const s = pres.addSlide();
  s.background = { color: C.white };
  slideHeader(s, "引流团单利弊分析", "为什么有些门店不愿意执行引流团单？");
  s.addText("门店常见顾虑", { x: 0.4, y: 1.25, w: 4.5, h: 0.4, fontSize: 14, bold: true, color: C.red, margin: 0 });
  const concerns = [
    { title: "「29.9元不赚钱，还要倒贴成本」", analysis: "引流产品本质是「获客成本」，不是利润产品。算总账：获客成本13元，转化1个169元卡赚160元，ROI 1:11" },
    { title: "「引流来的都是薅羊毛的，转化不了」", analysis: "不是客户问题，是转化能力问题。30分钟标准化服务+专业诊断+效果呈现，转化率可达30%+" },
    { title: "「服务流程太长，员工不愿意做」", analysis: "新团单已优化为30分钟标准化SOP，不拖沓。员工抵触是因为旧习惯，需要培训和考核" },
    { title: "「引流客太多，影响正常客户体验」", analysis: "引流客预约时段可错峰安排（工作日上午），不影响正常客户。且引流客是「增量」，不是「替代」" },
  ];
  concerns.forEach((c, i) => {
    const y = 1.72 + i * 0.95;
    card(s, 0.4, y, 4.5, 0.88, "FEF2F2");
    leftAccent(s, 0.4, y, 0.88, C.red);
    s.addText(c.title, { x: 0.57, y: y + 0.06, w: 4.2, h: 0.28, fontSize: 10, bold: true, color: C.dark, margin: 0 });
    s.addText(c.analysis, { x: 0.57, y: y + 0.36, w: 4.2, h: 0.48, fontSize: 9, color: C.gray, margin: 0 });
  });
  s.addText("引流团单利弊对比", { x: 5.1, y: 1.25, w: 4.5, h: 0.4, fontSize: 14, bold: true, color: C.dark, margin: 0 });
  const prosCons = [
    { aspect: "短期收益", con: "单客利润低（约13元）", pro: "获客成本远低于地推/广告" },
    { aspect: "客户质量", con: "部分客户只体验不转化", pro: "筛选出真正有需求的精准客户" },
    { aspect: "员工负担", con: "需要学习新SOP和话术", pro: "30分钟标准化，效率更高" },
    { aspect: "门店压力", con: "短期内到店量可能激增", pro: "可预约错峰，且是「增量」" },
    { aspect: "转化难度", con: "需要专业转化能力", pro: "转化后客户价值高（169-499元）" },
    { aspect: "长期价值", con: "需要持续投入维护", pro: "建立客户池，可持续复购和转介绍" },
  ];
  prosCons.forEach((r, i) => {
    const y = 1.72 + i * 0.58;
    const bg = i % 2 === 0 ? C.lightGray : C.white;
    s.addShape("rect", { x: 5.1, y, w: 4.4, h: 0.52, fill: { color: bg }, line: { width: 0 } });
    s.addText(r.aspect, { x: 5.25, y: y + 0.08, w: 1.2, h: 0.36, fontSize: 9.5, bold: true, color: C.dark, margin: 0 });
    s.addShape("rect", { x: 6.5, y: y + 0.08, w: 1.4, h: 0.36, fill: { color: "FEF2F2" }, line: { width: 0 } });
    s.addText(r.con, { x: 6.55, y: y + 0.08, w: 1.3, h: 0.36, fontSize: 8.5, color: C.red, margin: 0 });
    s.addShape("rect", { x: 8.0, y: y + 0.08, w: 1.4, h: 0.36, fill: { color: "F0FDF4" }, line: { width: 0 } });
    s.addText(r.pro, { x: 8.05, y: y + 0.08, w: 1.3, h: 0.36, fontSize: 8.5, color: C.primary, margin: 0 });
  });
  s.addShape("rect", { x: 0.4, y: 5.0, w: 9.2, h: 0.35, fill: { color: C.dark }, line: { width: 0 } });
  s.addText("核心结论：引流团单不是「要不要做」的问题，是「怎么做才能转化」的问题。", { x: 0.5, y: 5.05, w: 9, h: 0.28, fontSize: 10.5, bold: true, color: C.white, margin: 0 });
  footer(s, 22);
}

// ── Slide 23: 引流团单可能遇到的问题（新增）─────────────────────
{
  const s = pres.addSlide();
  s.background = { color: C.light };
  slideHeader(s, "引流团单执行中可能遇到的问题", "提前预判，提前准备");
  const problems = [
    { problem: "客户到店后要求退款/投诉", cause: "服务体验不好 / 与预期不符", solution: "严格执行30分钟SOP，不强推，用效果说话", prevention: "培训员工服务意识，建立客诉快速响应机制" },
    { problem: "引流客太多，正常客户预约不上", cause: "引流客和正常客户时段冲突", solution: "引流客预约工作日上午/下午时段，错峰安排", prevention: "建立预约管理系统，区分客户类型安排时段" },
    { problem: "员工不愿意服务引流客", cause: "觉得引流客「不值钱」/ 服务流程不熟悉", solution: "明确引流客转化后的提成机制", prevention: "设置引流客转化奖励，让员工看到长期价值" },
    { problem: "引流客转化率低", cause: "转化话术不到位 / 服务体验一般 / 没有跟进", solution: "复盘每个未转化客户的原因；优化话术和流程", prevention: "建立转化数据跟踪，每周复盘优化" },
    { problem: "差评风险", cause: "客户期望值过高 / 服务不到位 / 沟通不畅", solution: "服务前明确告知服务内容和时长", prevention: "管理客户预期，超预期服务，及时跟进" },
    { problem: "薅羊毛客户占比高", cause: "只体验不转化 / 多次购买体验卡", solution: "设置体验卡购买限制（新客专享）", prevention: "平台设置购买限制，门店做好转化" },
  ];
  problems.forEach((p, i) => {
    const x = 0.4 + (i % 2) * 4.8;
    const y = 1.25 + Math.floor(i / 2) * 1.35;
    card(s, x, y, 4.6, 1.25);
    s.addShape("rect", { x, y, w: 4.6, h: 0.28, fill: { color: C.red, transparency: 80 }, line: { width: 0 } });
    s.addText("问题：" + p.problem, { x: x + 0.1, y: y + 0.04, w: 4.4, h: 0.24, fontSize: 9.5, bold: true, color: C.dark, margin: 0 });
    s.addText("原因：" + p.cause, { x: x + 0.1, y: y + 0.32, w: 4.4, h: 0.22, fontSize: 8.5, color: C.gray, margin: 0 });
    s.addText("解决：" + p.solution, { x: x + 0.1, y: y + 0.55, w: 4.4, h: 0.3, fontSize: 8.5, color: C.primary, margin: 0 });
    s.addText("预防：" + p.prevention, { x: x + 0.1, y: y + 0.85, w: 4.4, h: 0.36, fontSize: 8.5, color: C.secondary, italic: true, margin: 0 });
  });
  footer(s, 23);
}

// ── Slide 24: 抖音来客后台客资中心使用教程（新增）────────────────
{
  const s = pres.addSlide();
  s.background = { color: C.light };
  slideHeader(s, "模块五：抖音来客后台 · 客资中心使用教程", "如何高效管理引流客户？");
  s.addText("客资中心核心功能", { x: 0.4, y: 1.25, w: 4.5, h: 0.4, fontSize: 14, bold: true, color: C.primary, margin: 0 });
  const features = [
    { title: "客户信息采集", desc: "自动抓取购买团单的客户手机号、姓名、购买时间" },
    { title: "客户状态跟踪", desc: "标记客户状态：已预约/已到店/已服务/已转化" },
    { title: "智能分配", desc: "自动分配给就近门店或指定美容顾问" },
    { title: "数据分析", desc: "查看转化率、到店率、复购率等核心数据" },
    { title: "消息触达", desc: "一键发送短信/站内信，提醒客户预约" },
  ];
  features.forEach((f, i) => {
    const y = 1.72 + i * 0.72;
    card(s, 0.4, y, 4.5, 0.65);
    s.addShape("ellipse", { x: 0.55, y: y + 0.12, w: 0.4, h: 0.4, fill: { color: C.secondary, transparency: 30 }, line: { width: 0 } });
    s.addText(String(i + 1), { x: 0.55, y: y + 0.14, w: 0.4, h: 0.36, fontSize: 16, bold: true, color: C.primary, align: "center", margin: 0 });
    s.addText(f.title, { x: 1.05, y: y + 0.08, w: 3.7, h: 0.28, fontSize: 11, bold: true, color: C.dark, margin: 0 });
    s.addText(f.desc, { x: 1.05, y: y + 0.36, w: 3.7, h: 0.26, fontSize: 9, color: C.gray, margin: 0 });
  });
  s.addText("操作步骤", { x: 5.1, y: 1.25, w: 4.5, h: 0.4, fontSize: 14, bold: true, color: C.dark, margin: 0 });
  const steps = [
    { title: "登录抖音来客后台", desc: "打开抖音来客APP或PC端，使用门店账号登录" },
    { title: "进入「客资中心」", desc: "首页 → 数据中心 → 客资中心，查看所有客户列表" },
    { title: "查看新客户", desc: "筛选「今日新增」或「未跟进」，查看新购买团单的客户" },
    { title: "点击「立即跟进」", desc: "查看客户详情（手机号/购买项目/购买时间）" },
    { title: "拨打电话邀约", desc: "使用系统内置拨号功能，或复制手机号用门店电话拨打" },
    { title: "标记客户状态", desc: "跟进后标记为「已预约」或「待跟进」，设置下次跟进提醒" },
    { title: "客户到店后", desc: "标记为「已到店」，服务完成后标记「已服务」" },
    { title: "转化后标记", desc: "客户购买其他团单或会员卡后，标记为「已转化」" },
  ];
  steps.forEach((st, i) => {
    const y = 1.72 + i * 0.45;
    card(s, 5.1, y, 4.4, 0.42);
    s.addShape("ellipse", { x: 5.2, y: y + 0.06, w: 0.3, h: 0.3, fill: { color: C.primary }, line: { width: 0 } });
    s.addText(String(i + 1), { x: 5.2, y: y + 0.08, w: 0.3, h: 0.26, fontSize: 12, bold: true, color: C.white, align: "center", margin: 0 });
    s.addText(st.title, { x: 5.6, y: y + 0.04, w: 2.5, h: 0.2, fontSize: 10, bold: true, color: C.dark, margin: 0 });
    s.addText(st.desc, { x: 5.6, y: y + 0.22, w: 3.8, h: 0.18, fontSize: 8.5, color: C.gray, margin: 0 });
  });
  footer(s, 24);
}

// ── Slide 25: 客资中心使用技巧（新增）──────────────────────────
{
  const s = pres.addSlide();
  s.background = { color: C.white };
  slideHeader(s, "客资中心使用技巧", "提升转化效率的5个关键点");
  const tips = [
    { title: "黄金24小时", desc: "客户购买团单后24小时内必须电话跟进，超过48小时转化率下降50%", icon: "⏰", color: C.red },
    { title: "预约时段选择", desc: "工作日上午10-11点、下午3-5点邀约成功率最高，避开午休和下班时间", icon: "📅", color: C.secondary },
    { title: "客户分层管理", desc: "按购买金额分层：29.9元引流客/58元锁客/169元+利润客，不同话术策略", icon: "🎯", color: C.primary },
    { title: "跟进记录完整", desc: "每次跟进记录客户反馈、顾虑、下次跟进时间，避免重复沟通", icon: "📝", color: C.accent },
    { title: "数据复盘", desc: "每周查看客资中心数据：新增客户数、预约率、到店率、转化率，持续优化", icon: "📊", color: C.gold },
  ];
  tips.forEach((t, i) => {
    const x = 0.4 + (i % 2) * 4.8;
    const y = 1.25 + Math.floor(i / 2) * 1.35;
    card(s, x, y, 4.6, 1.25);
    s.addShape("ellipse", { x: x + 0.15, y: y + 0.15, w: 0.6, h: 0.6, fill: { color: t.color, transparency: 20 }, line: { width: 0 } });
    s.addText(t.icon, { x: x + 0.15, y: y + 0.2, w: 0.6, h: 0.5, fontSize: 24, align: "center", margin: 0 });
    s.addText(t.title, { x: x + 0.85, y: y + 0.2, w: 3.6, h: 0.35, fontSize: 14, bold: true, color: t.color, margin: 0 });
    s.addText(t.desc, { x: x + 0.15, y: y + 0.85, w: 4.3, h: 0.55, fontSize: 10, color: C.gray, margin: 0 });
  });
  s.addShape("rect", { x: 0.4, y: 4.9, w: 9.2, h: 0.45, fill: { color: C.dark }, line: { width: 0 } });
  s.addText("重要提醒：客资中心的数据是门店的核心资产，务必每日查看、及时跟进、完整记录。", { x: 0.5, y: 4.95, w: 9, h: 0.2, fontSize: 10, bold: true, color: C.white, margin: 0 });
  s.addText("客资跟进率低于80%的门店，引流效果会大打折扣！", { x: 0.5, y: 5.15, w: 9, h: 0.18, fontSize: 9.5, color: C.secondary, margin: 0 });
  footer(s, 25);
}


pres.writeFile({ fileName: "/Users/flyingspur/Desktop/荟宝团单升级培训_完整版.pptx" })

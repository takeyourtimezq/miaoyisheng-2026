...
# ===== 三、启动成本与行动 =====
doc.add_page_break()
heading(doc, '三、启动成本与行动建议', 1)

heading(doc, '3.1 启动成本估算（铁路局30-50㎡）', 2)
t5 = doc.add_table(rows=8, cols=2, style='Table Grid')
borders(t5)
for j, h in enumerate(['项目', '预估金额']):
    set_cell(t5.rows[0].cells[j], h, bold=True, size=9, color=RGBColor(255,255,255))
header_bg(t5)
for i, (k, v) in enumerate([
    ('押金（押二付一）', '5,000-15,000元'),
    ('装修', '30,000-80,000元'),
    ('设备（仪器+家具）', '20,000-50,000元'),
    ('证照', '2,000-5,000元'),
    ('首月人工（1-2人）', '5,000-14,000元'),
    ('首批物料', '3,000-8,000元'),
    ('合计', '6.5万-17.2万元'),
]):
    set_cell(t5.rows[i+1].cells[0], k, size=9, align=WD_ALIGN_PARAGRAPH.LEFT)
    set_cell(t5.rows[i+1].cells[1], v, size=9)
    if i == 6:
        for c in t5.rows[i+1].cells:
            for p in c.paragraphs:
                for r in p.runs:
                    r.font.bold = True

heading(doc, '3.2 行动清单', 2)
t6 = doc.add_table(rows=6, cols=2, style='Table Grid')
borders(t6)
for j, h in enumerate(['事项', '时间']):
    set_cell(t6.rows[0].cells[j], h, bold=True, size=9, color=RGBColor(255,255,255))
header_bg(t6)
for i, (k, v) in enumerate([
    ('实地考察铁路局商圈，锁定2-3个候选铺位', '本周内'),
    ('走访竞品（良卡瘦身、太百减肥体验中心），了解客单价和客流量', '考察期间'),
    ('58同城/安居客筛选30-50㎡商铺出租信息', '本周内'),
    ('测算详细启动成本和盈亏平衡点', '考察后3天'),
    ('了解乌鲁木齐健康减脂类经营许可证办理流程', '签约前'),
]):
    set_cell(t6.rows[i+1].cells[0], k, size=9, align=WD_ALIGN_PARAGRAPH.LEFT)
    set_cell(t6.rows[i+1].cells[1], v, size=9)

# ===== 四、总结 =====
doc.add_paragraph()
heading(doc, '四、总结', 1)

t7 = doc.add_table(rows=2, cols=1, style='Table Grid')
borders(t7)
s = OxmlElement('w:shd')
...
summary = (
    '基于美团和高德地图竞品数据，乌鲁木齐瘦身塑形类商户共梳理出12家，其中铁路局/新市区占6家（50%），'
    '为全城竞品最密集区域。竞品集中不等于不能做——恰恰说明该区域市场需求真实、已被验证。'
    '\n\n'
    '轻刻时代30-50㎡瘦身塑形首店，建议选址铁路局商圈。'
    '核心原因有三：①市场已验证（6家竞品持续在营）；②成本可控（月租金2,400-7,500元，启动资金约6.5-17万）；'
    '③铺型匹配（30-50㎡为该商圈主流铺型）。'
    '\n\n'
    '友好路商圈留作品牌旗舰店（样板店跑通后），民俗街商圈作为差异化选项观察。'
    '\n\n'
    '下一步：本周内实地考察铁路局，锁定铺位，测算盈亏平衡。'
)
set_cell(t7.rows[0].cells[0], summary, size=10.5, align=WD_ALIGN_PARAGRAPH.LEFT)

path = '/Users/flyingspur/.qclaw/workspace-agent-6f07984c/轻刻时代_乌鲁木齐选址分析_V3.docx'
doc.save(path)
print(f'OK: {path}')
